  ...
  var mo = {
    getDefaultPopupInfo: function(
      ...
      if (layerDefinition.displayField) {
        var displayField = this._getRealFieldName(
          layerDefinition.displayField,
          layerDefinition
        );
      ...
      if (!showShapeField) {
        filteredServiceFieldInfos = array.filter(
          allServiceFieldInfos,
          lang.hitch(this, function(serviceFieldInfo) {
            return serviceFieldInfo.type !== "esriFieldTypeGeometry";
          })
        );
      }
      var portalFieldInfos = array.map(
        filteredServiceFieldInfos,
        lang.hitch(this, function(serviceFieldInfo) {
          var portalFieldInfo = jimuUtils.getDefaultPortalFieldInfo(
            serviceFieldInfo
          );
    ...
    getPortalFieldInfosWithoutShape: function(
      layerDefinition,
      portalFieldInfos
    ) {
      var result = array.filter(
        portalFieldInfos,
        lang.hitch(this, function(portalFieldInfo) {
          var serviceFieldInfo = jimuUtils.getFieldInfoByFieldName(
            layerDefinition.fields,
            portalFieldInfo.fieldName
          );
          return serviceFieldInfo.type !== "esriFieldTypeGeometry";
        })
      );
    ...
    getPopupInfoByAttributes: function(layerDefinition, attributes) {
      ...
      popupInfo.fieldInfos = array.filter(
    ...
    getConfigWithValidDataSource: function(config) {
      ...
      validConfig.queries = array.filter(
        config.queries,
    ...
    overridePopupTemplateMethodGetAttachments: function(
      popupTtemplate,
      serviceUrl,
      objectIdField
    ) {
    ...
    isSupportObjectIds: function(layerDefinition) {
      ...
      return (
        !!objectIdField &&
        (currentVersion >= 10.0 ||
          layerDefinition.hasOwnProperty("typeIdField"))
      );
    },
    ...
    getQueryType: function(layerDefinition) {
      var queryType = -1;
      if (
        this.isServiceSupportsOrderBy(layerDefinition) &&
    ...
    getWebMapPopupInfoByUrl: function(webMapItemData, layerUrl) {
      ...
      array.some(
        ...
        lang.hitch(this, function(info) {
          if (info && info.url) {
            var urlWithoutProtocol = portalUrlUtils.removeProtocol(
              info.url.replace(/\/*$/g, "")
            );
            if (info.popupInfo) {
    ...
    getPopupInfoForRelatedLayer: function(
      webMapItemData,
      layerUrl,
      layerDefinition
    ) {
